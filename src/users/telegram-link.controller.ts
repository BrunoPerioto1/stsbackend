...
export class TelegramLinkController {
  ...
  confirmLink(@Body() body: { code: string; telegramUserId: number }) {
    const { code, telegramUserId } = body;
    const userId = telegramLinkCodes[code];
    if (!userId) {
      return { success: false, message: 'Código inválido ou expirado.' };
    }
    // Aqui você deve salvar o telegramUserId no perfil do usuário (DB)
    // Exemplo: await usersService.vincularTelegram(userId, telegramUserId)
